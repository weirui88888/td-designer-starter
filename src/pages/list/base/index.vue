    ...
    <t-card class="list-card-container" :bordered="false">
      ...
      >
        ...
        <template #op="{ row, rowIndex }">
          <t-space>
            <t-link theme="primary" @click="handleClickDetail(row.no)"> {{ t('pages.listBase.detail') }}</t-link>
            <t-link theme="danger" @click="handleClickDelete(rowIndex)"> {{ t('pages.listBase.delete') }}</t-link>
          </t-space>
        </template>
      </t-table>
    </t-card>
...
const handleClickDetail = (id: string) => {
  router.push(`/list/base-info/${id}`);
};
const handleSetupContract = () => {
  router.push('/list/base-info');
};
